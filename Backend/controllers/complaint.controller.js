...
export const submitComplaint = async (req, res) => {
  try {
    ...
    const activeNearby = nearbyAll.filter(
      c =>
        haversineDistance(newLat, newLon, c.latitude, c.longitude) <= RADIUS_KM
    )
    ...
    if (activeNearby.length > 0) {
      ...
      const distanceMeters = Math.round(
        haversineDistance(newLat, newLon, closest.latitude, closest.longitude) *
          1000
      )
      ...
      return res.status(409).json({
        ...
        data: {
          ...
          userMessage: [
            ...
            `📅 Submitted on: ${new Date(closest.createdAt).toLocaleDateString(
              'en-IN',
              {
                day: 'numeric',
                month: 'long',
    ...
    await ComplaintStatus.create({
      ...
      status: 'Pending',
      statusHistory: [
        { status: 'Pending', note: 'Complaint submitted by user' }
      ]
    })
